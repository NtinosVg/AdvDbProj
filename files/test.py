...
incomela_df=geocode_df.join(income_df.hint("SHUFFLE_HASH"),[(income_df.ZIP == geocode_df.ZIPcode)],'inner')
...
incomela_df=incomela_df.join(Inc_df.hint("SHUFFLE_HASH"),[(income_df.ZIP == Inc_df.ZIPCode)],'inner').select("lat1","lon1","ZIP")

result_df=crime_df.join(incomela_df.hint("SHUFFLE_HASH"),[(crime_df.LON==incomela_df.lon1) & (crime_df.LAT==incomela_df.lat1)],'inner')
...
result_df =result_df.join(mapping_df.hint("SHUFFLE_HASH"),'Vict Descent','left')

result_df=result_df.select("Victim Descent","count") \
